...
//    auto max_val = at::zeros({batch, channel, height},torch::CUDA(torch::kFloat));
    auto max_val = at::zeros({batch, channel, height},input.options().dtype(at::kFloat));
//    auto max_ind = at::zeros({batch, channel, height},torch::CUDA(torch::kLong));
    auto max_ind = at::zeros({batch, channel, height},input.options().dtype(at::kLong));
...
//    auto gt_mask    = at::zeros({batch, channel, height},torch::CUDA(torch::kByte));
    auto gt_mask    = at::zeros({batch, channel, height},input.options().dtype(at::kByte));

//    auto max_temp   = at::zeros({batch, channel, height},torch::CUDA(torch::kFloat));
    auto max_temp   = at::zeros({batch, channel, height},input.options().dtype(at::kFloat));
