...
std::vector<at::Tensor> pool_backward(
    ...
    auto max_val = at::zeros({batch, channel, width},torch::CUDA(torch::kFloat));
    auto max_ind = at::zeros({batch, channel, width},torch::CUDA(torch::kLong));
    ...
    auto gt_mask    = at::zeros({batch, channel, width},torch::CUDA(torch::kByte));
    auto max_temp   = at::zeros({batch, channel, width},torch::CUDA(torch::kFloat));
